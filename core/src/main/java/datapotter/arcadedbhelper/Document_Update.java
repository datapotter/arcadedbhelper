...
import datapotter.datahelper.MapWrites;
...
public class Document_Update {
     ...
     */
    @SafeVarargs
    public final Document_Update from(datapotter.datahelper.DataHelper_I<?> dataHelper, Field_I<?, ?>... fields) {
     ...
     * @return this Document_Update instance
     */
    public Document_Update from(datapotter.datahelper.DataHelper_I<?> dataHelper, String... fields) {
        if (fields == null || fields.length == 0) {
            // Map all fields
    ...
    public <T> Document_Update __(datapotter.datahelper.Field_I<?, T> field, Supplier<T> valueProvider) {
        return __(field.name(), valueProvider);
    }

    // Type-safe Field_I with direct value
    public <T> Document_Update __(datapotter.datahelper.Field_I<?, T> field, T value) {
